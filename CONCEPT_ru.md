...
2. **Режем на чанки.** Каждый файл делится на осмысленные фрагменты — «чанки». Код и конфиги — по структуре, документы — по тексту (об этом ниже).
3. **Считаем эмбеддинги.** Локальная модель (`sentence-transformers`) превращает каждый чанк в вектор чисел — «координаты смысла». Та же модель эмбеддит и поисковый запрос.
4. **Сохраняем.** Вектор пишется в базу, а рядом — метаданные чанка (репозиторий, путь, язык, символ, строки). Каждому репозиторию — свой отдельный датасет.
5. **Ищем.** Запрос тоже превращается в вектор, база находит ближайшие по смыслу чанки в нужных датасетах, результаты объединяются и ранжируются.
6. **Показываем.** Ответ выглядит как `repo/path:строки, символ, оценка, сниппет` — сразу понятно, куда идти.

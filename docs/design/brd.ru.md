...
> **Запись о намерении, а не о нынешней сборке.** Документ говорит, что
> требовалось на момент его написания. Многое сбылось; решение о
> хранилище — нет. **Tensorus v1 заменён на LanceDB решением ADR-7
> (август 2026), код удалён 2026-08-21**, поэтому все требования FR-4xx,
> все REST-эндпоинты и оба имени бэкендов ниже описывают устройство,
> которого больше нет. `LocalStore` исчез вместе с ним.
>
> Оставлено как есть, а не переписано, и это сознательно: протащить новое
> решение через четырнадцать разделов значило бы превратить запись о
> желаемом в утверждение о сущем и потерять возможность увидеть, что
> изменилось. Что система делает сегодня — в
> [README.md](../../README.md), он измерен и проверяется тестами против
> CLI; почему сменилось хранилище — в
> `../adr/adr-007-post-mvp-storage.md`.
...
> Полностью снято решением ADR-7. Уцелела абстракция `VectorStore`
> (FR-401) — единственное здесь, что осталось верным; всё
> остальное — `TensorusStore`, `LocalStore`, REST-эндпоинты,
> ключ API — удалено вместе с бэкендом.

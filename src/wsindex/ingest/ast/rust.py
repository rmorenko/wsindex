...
from wsindex.ingest.ast.core import Span, gap_spans, line_span, mark_covered, symbol_name
...
def _extend_back(siblings: list[Node], *, index: int, start_line: int) -> int:
    ...
    for prev in reversed(siblings[:index]):
        if prev.type not in ("attribute_item", "line_comment"):
            break
        prev_start, prev_end = line_span(prev)
...
def _rust_def_span(siblings: list[Node], *, index: int, covered: list[bool], symbol: str) -> Span:
...
def spans(root: Node, lines: list[str], covered: list[bool]) -> list[Span]:
    ...
    for i, child in enumerate(children):
        if child.type in _RUST_DEFS:
            ...
            impl_start, impl_end = line_span(child)

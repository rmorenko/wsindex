...
- **ingest.ast_chunker** — splits via tree-sitter. For code — functions/classes/methods/blocks; for configs (TOML/YAML/JSON/XML/Dockerfile) — by structural nodes (tables/keys/sections/stages/element runs). Fills in `symbol`, `node_type`, `start_line`, `end_line`.

...
def test_markdown_sections() -> None:
    ...
    chunks = chunk_markdown(example, repo=REPO, path=PATH, lang="markdown")
    assert len(chunks) == 3
...
def test_plain_short_text_single_chunk() -> None:
    example = dedent("""\
         ...
         fourth line
     """)
    chunks = chunk_plain(example, repo=REPO, path=PATH, lang="text")
    assert len(chunks) == 1
...
def test_markdown_blank_sections_skipped() -> None:
    assert chunk_markdown("", repo=REPO, path=PATH, lang="markdown") == []
    # Blank preamble before the first header must not become an empty chunk.
    chunks = chunk_markdown("\n\n# One\nbody", repo=REPO, path=PATH, lang="markdown")
    assert len(chunks) == 1
...
def test_overlap_must_be_less_than_window() -> None:
    with pytest.raises(ValueError):
        chunk_plain("x", repo="r", path="p", lang="text", window=10, overlap=10)
...
def test_metadata_flows_through() -> None:
    example = dedent("""\
             ...
             fourth line
         """)
    chunks = chunk_text(example, repo=REPO, path=PATH, lang="rst")
    for chunk in chunks:
        assert chunk.kind == Kind.DOC
        assert chunk.repo == REPO
        assert chunk.path == PATH
        assert chunk.lang == "rst"
    chunks = chunk_text(example, repo=REPO, path=PATH, lang="markdown")
    for chunk in chunks:
        assert chunk.kind == Kind.DOC
        assert chunk.repo == REPO
        assert chunk.path == PATH

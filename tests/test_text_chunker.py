...
def test_markdown_sections() -> None:
    ...
    chunks = chunk_markdown(
        example, SourceFile(repo=REPO, path=PATH, lang="markdown", kind=Kind.DOC)
    )
    assert len(chunks) == 3
...
def test_plain_short_text_single_chunk() -> None:
    example = dedent("""\
         ...
         fourth line
     """)
    chunks = chunk_plain(example, SourceFile(repo=REPO, path=PATH, lang="text", kind=Kind.DOC))
    assert len(chunks) == 1
...
def test_markdown_blank_sections_skipped() -> None:
    assert (
        chunk_markdown("", SourceFile(repo=REPO, path=PATH, lang="markdown", kind=Kind.DOC)) == []
    )
    # Blank preamble before the first header must not become an empty chunk.
    chunks = chunk_markdown(
        "\n\n# One\nbody", SourceFile(repo=REPO, path=PATH, lang="markdown", kind=Kind.DOC)
    )
    assert len(chunks) == 1
...
def test_overlap_must_be_less_than_window() -> None:
    with pytest.raises(ValueError):
        chunk_plain(
            "x", SourceFile(repo="r", path="p", lang="text", kind=Kind.DOC), window=10, overlap=10
        )
...
def test_metadata_flows_through() -> None:
    example = dedent("""\
             ...
             fourth line
         """)
    chunks = chunk_text(example, SourceFile(repo=REPO, path=PATH, lang="rst", kind=Kind.CODE))
    for chunk in chunks:
        assert chunk.repo == REPO
        assert chunk.path == PATH
        assert chunk.lang == "rst"
        assert chunk.kind == Kind.CODE
    chunks = chunk_text(example, SourceFile(repo=REPO, path=PATH, lang="markdown", kind=Kind.DOC))
    for chunk in chunks:
